...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def segmentation(self): # Сегментация
        ...
        self.dst = cv2.bitwise_and(img, img, mask=mask)

        ## Display it
        cv2.imwrite("dst.png", self.dst)
        self.scene_2 = QGraphicsScene(self.graphicsView_2)
        self.scene_2.clear()
    ...
    def glcm_all(self):
        path = 'C:\\Users\\bulig\\PycharmProjects\\pythonProject\\test'
        list = os.listdir('C:\\Users\\bulig\\PycharmProjects\\pythonProject\\test')
    ...
    def SVM(self): # Классификатор опорных векторов
        ...
        cell1 = pd.read_csv("glcm_train1.csv", delimiter=';')
        cell1 = cell1.drop(columns=cell1.columns[0])
        cell1["class"] = 0
        cell2 = pd.read_csv("glcm_train2.csv", delimiter=';')
        cell2 = cell2.drop(columns=cell2.columns[0])
        cell2["class"] = 1
        cell3 = pd.read_csv("glcm_train3.csv", delimiter=';')
        cell3 = cell3.drop(columns=cell3.columns[0])
        cell3["class"] = 2
        cells = pd.concat([cell1,cell2,cell3])
        cells = cells.apply(lambda x: pd.to_numeric(x.astype(str).str.replace(',', '.'), errors='coerce'))
        print(cells)

        # Загрузка тестовой выборки
        test_cell1 = pd.read_csv("glcm_test1.csv", delimiter=';')
        test_cell1 = test_cell1.drop(columns=test_cell1.columns[0])
        test_cell1["class"] = 0
        test_cell2 = pd.read_csv("glcm_test2.csv", delimiter=';')
        test_cell2 = test_cell2.drop(columns=test_cell2.columns[0])
        test_cell2["class"] = 1
        test_cell3 = pd.read_csv("glcm_test3.csv", delimiter=';')
        test_cell3 = test_cell3.drop(columns=test_cell3.columns[0])
        test_cell3["class"] = 2
        test_cells = pd.concat([test_cell1, test_cell2,test_cell3])
