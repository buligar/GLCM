...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def addFunctions(self):
        ...
        self.pushButton_3.clicked.connect(lambda: self.glcm_rgb_one()) # GLCM-RGB
        self.pushButton_4.clicked.connect(lambda: self.glcm_rgb_all()) # GLCM-RGB обучающая и тестовая выборка
    ...
    def glcm_rgb_one(self):
        ...
        self.num_cells = np.array(range(1, num + 1)).flatten()  # Массив кол-ва изображений
        head = np.array(range(1, 4*24 + 1)).flatten()
        mat = np.matrix(head)
        df = pd.DataFrame(mat)
        df.to_csv('glcm_all.csv', mode='a', header=False,index=False, float_format="%.5f",
        ...
        for i in range(num):
            ...
            for k in range(4):
                color = k
                self.grayscale = cv2.imread(self.file_path)
                if color == 0:
                    self.grayscale[:, :, 0] = 0
                    self.grayscale[:, :, 1] = 0
                    cv2.imwrite('r.bmp', self.grayscale)
                ...
                value = self.spinBox.value()  # Значение Расстояние смежности в spinbox
                # Distances = np.arange(1, value + 1, 1)
                glcm = greycomatrix(self.grayscale, distances=[value],
                                    angles=[0, np.pi / 4, np.pi / 2, 3 * np.pi / 4],
                                    levels=256,
                                    symmetric=True, normed=True)  # Построение МПС
                # print(glcm)
